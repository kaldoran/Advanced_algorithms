...
Solution tsp_brute_force(Graph g) {
	...
	add_node(tsp_final_solution,g->nodes[0],0);
	tsp_final_solution2 = brute_force(tsp_final_solution, g->nodes[0], g->count_nodes, 0);
...
Solution brute_force(Solution s, Node n, int total_node, int last_cost) {
	...
	while(i < n->count_subnodes) {
		...
		copy_solution(sub_solution, s);puts("fuck");
		brute_force(sub_solution, n->subnodes[i], total_node, n->cost[i]);
		puts("fuckDDD");
		
		if(sub_solution != NULL) {

			tab_solution = (Solution*)realloc(tab_solution, dim_tab*sizeof(Solution));
			...
			tab_solution[dim_tab] = new_solution ();print_solution(sub_solution);
